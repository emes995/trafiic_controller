...
class TrackableSpace:
    ...
    def addControllable(self, controllableItem: TrackableItem, replace: bool = True):
        if not replace:
        ...
        if self._maxPopulation > -1:
            if len(self) >= self._maxPopulation:
                raise MaxTrackableSpacePopulationException(f'Population already reached its limit'
